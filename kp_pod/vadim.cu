...
void create_tetrahedral(std::vector<triangle>& trigs, const double& radius, const vec3& c_coords, const vec3& colors) { // +++++++
    std::vector<vec3> points(4);
    double tetrahedralAngle = M_PI * 109.4712 / 180;
    double segmentAngle = M_PI * 2 / 3;
    double currentAngle = 0.0;
    
    points[0] = {0, radius, 0};
    for(int i = 1; i <= 3; i++) {
        points[i] = {radius * sin(currentAngle) * sin(tetrahedralAngle),
                     radius * cos(tetrahedralAngle),
                     radius * cos(currentAngle) * sin(tetrahedralAngle)};
...
int main(int argc, char *argv[]) {
    ...
    std::cin >> c_verticles.x >> c_verticles.y >> c_verticles.z >> col_values.x >> col_values.y >> col_values.z >> radius;
    ...
    col_values.y *= 255.0;
    col_values.z *= 255.0;
    create_tetrahedral(trigs, radius, c_verticles, col_values);
    ...
    std::cin >> temp;
    std::cin >> col_values.x >> col_values.y >> col_values.z;
    std::cin >> temp;
    col_values.x *= 255.0;
    col_values.y *= 255.0;
    col_values.z *= 255.0;
